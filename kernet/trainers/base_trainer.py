...
class BaseTrainer(torch.nn.Module):
  ...
  def load(self, model_name='net.pth'):
    ...
    if not os.path.isfile(filename):
      logger.warning('Did not load {} since {} does not exist.'.format(model_name, filename))
      return
    ...
    try:
      ...
      logger.warning('Missing keys from model checkpoint:\n' + ', '.join(sorted(missing)))
      logger.warning('Extra keys from model checkpoint:\n' + ', '.join(sorted(extra)))
      logger.warning('Loading only the matched keys from model checkpoint...')
    ...
    if self.opt.is_train:
      if self.opt.schedule_lr: # TODO potentially overrides new params
        try:
          ...
          logger.warning('Did not find scheduler state dict from checkpoint. Not loading it...')
      ...
      logger.debug('Now starting from epoch {}...'.format(self.start_epoch+1))
