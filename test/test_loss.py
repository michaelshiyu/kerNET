...
class SRSLossTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.input_4D = torch.tensor(
            [[1, 3.], [.5, -2], [9, .1]]).view(3, 1, 2, 1).to(self.device)
    ...
    def test_srsloss_instance1_2dinput(self, ):
        def map_input(x): return torch.sum(
            (x - x.unsqueeze(1)) ** 2, dim=(-1)).unsqueeze(-1)

        def map_target(x): return x * x.unsqueeze(1)
        loss_fn = torch.nn.MSELoss(reduction='mean')
        srsloss_fn = SRSLoss(map_input=map_input,
                             map_target=map_target, loss=loss_fn)
    ...
    def test_srsloss_instance1_3dinput(self, ):
        def map_input(x): return torch.sum(
            (x - x.unsqueeze(1)) ** 2, dim=(-1, -2)).unsqueeze(-1)
        ...
        input = torch.tensor([[[1., 2], [1, 3]], [[3, 1], [6, 4]], [
                             [5, 6], [0, 0]]]).to(self.device)
        target = torch.tensor([[1.], [-1], [-1]]).to(self.device)
    ...
    def test_srsloss_instance1_4dinput(self, ):
        def map_input(x): return torch.sum((x - x.unsqueeze(1))
                                           ** 2, dim=(-1, -2, -3)).unsqueeze(-1)
    ...
    def test_srs_alignment_neo(self, ):
        self.assertRaises(ValueError, srs_alignment_neo,
                          phi=self.phi, n_classes=self.n_classes)

    def test_srs_upper_tri_alignment(self, ):
        srsloss_fn = srs_upper_tri_alignment(
    ...
    def test_srs_upper_tri_alignment_neo(self, ):
        self.assertRaises(ValueError, srs_upper_tri_alignment_neo,
                          phi=self.phi, n_classes=self.n_classes)
    ...
    def test_srs_log_contrastive_neo(self, ):
        srsloss_fn = srs_log_contrastive_neo(
            self.phi, n_classes=self.n_classes)
        for target in [self.target1, self.target2]:
            res = srsloss_fn(self.input, target)
    ...
    def test_srs_alignment_neo_using_another_phi(self, ):
        srsloss_fn = srs_alignment_neo(
            self.another_phi, n_classes=self.n_classes)
        for target in [self.target1, self.target2]:
            res = srsloss_fn(self.input, target)
    ...
    def test_srs_upper_tri_alignment_neo_using_another_phi(self, ):
        srsloss_fn = srs_upper_tri_alignment_neo(
            self.another_phi, n_classes=self.n_classes)
        for target in [self.target1, self.target2]:
            res = srsloss_fn(self.input, target)
            self.assertAlmostEqual(res.cpu().numpy(), 0.1405, places=4)
    ...
    def test_srs_contrastive_neo_using_another_phi(self, ):
        srsloss_fn = srs_contrastive_neo(
            self.another_phi, n_classes=self.n_classes)
        for target in [self.target1, self.target2]:
            res = srsloss_fn(self.input, target)
    ...
    def test_srs_log_contrastive_using_another_phi(self, ):
        srsloss_fn = srs_log_contrastive(
            self.another_phi, n_classes=self.n_classes)
        for target in [self.target1, self.target2]:
            res = srsloss_fn(self.input, target)
