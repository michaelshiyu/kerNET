...
class kLinearLayerTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.klinear = kLinear(out_features=10, kernel='gaussian',
                               evaluation='indirect', centers=self.input).to(self.device)
        ...
        self.toy_klinear1.linear.weight.data = torch.tensor(
            [[.1, .2], [.5, .7]]).to(self.device)
        self.toy_klinear1.linear.bias.data = torch.tensor(
            [0., 0]).to(self.device)
        ...
        self.toy_klinear2.linear.weight.data = torch.tensor(
            [[1.2, .3], [.2, 1.7]]).to(self.device)
        self.toy_klinear2.linear.bias.data = torch.tensor(
            [.1, .2]).to(self.device)
    ...
    def test_two_layer_gaussian_indirect_forward(self, ):
        ...
        for input in [input2d, input3d, input4d]:
            ...
            # stop grad flow through layer2 centers to layer1
            self.toy_klinear2.centers = self.toy_klinear1(input).detach()
    ...
    def test_single_model_equals_committee(self):
        ...
        committee_output = self.klinear_committee(
            self.input).detach().cpu().numpy()
        self.assertTrue(np.allclose(
            single_model_output,
            committee_output,
            atol=1e-5
    ...
    def test_single_model_equals_committee_in_backward_pass(self):
        ...
        committee_optimizer = torch.optim.Adam(
            self.klinear_committee.parameters())
    ...
    def test_grad_computation_is_correct(self):
        def map_input(x): return self.toy_klinear2.phi.get_k_mtrx(x, x)

        def map_target(x): return self.toy_klinear2.phi.get_ideal_k_mtrx(
            x, x, n_classes=2)
