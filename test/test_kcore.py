...
class PhiTest(unittest.TestCase):
    ...
    def test_gaussian_phi_forward(self):
        ...
        input3d = torch.tensor(
            [[[.1, .5, .3]], [[0, .1, -.1]]]).to(self.device)
        centers3d = torch.tensor(
            [[[0, -.2, .1]], [[.1, .2, .3]]]).to(self.device)
        input4d = torch.tensor(
            [[[[.1, .5, .3]]], [[[0, .1, -.1]]]]).to(self.device)
        centers4d = torch.tensor(
            [[[[0, -.2, .1]]], [[[.1, .2, .3]]]]).to(self.device)
        ...
        for input, centers in zip(
            ...
            self.assertTrue(np.allclose(
                res.cpu().numpy(),
                np.array([[0.763379494, 0.955997481],
                          [0.937067463, 0.913931185]])
            ))
    ...
    def test_nn_tanh_phi_get_k_mtrx(self):
        ...
        self.assertTrue(np.allclose(
            ...
            np.array([[-0.506393473, 0.915915961],
                      [-0.949929847, -0.181622650]])
        ))
    ...
    def test_nn_relu_phi_get_k_mtrx(self):
        ...
        self.assertTrue(np.allclose(
            res.cpu().numpy(),
            np.array([[0.50709255283711, 0.9035079029052513],
                      [0, 0.5345224838248488]])
        ))
