...
ACT=reapen;
LOSS=xe;
SDIR=./checkpoints/fashionmnist_basic/${LOSS}/seed${SD}/e2e_nTrain${TR_SIZE}_nVal${V_SIZE}

CUDA_VISIBLE_DEVICES=${CUDA} python kernet/examples/train.py --dataset ${DATA} --model ${M} --loss ${LOSS} --lr .1 --activation ${ACT} --optimizer sgd --weight_decay ${WDECAY} --seed ${SD} --print_freq 1 --n_epochs 200 --loglevel info --n_classes 10 --augment_data ${AUG} --in_channels ${IN_CH} --batch_size ${BATCH}  --save_dir ${SDIR}_1 --max_trainset_size ${TR_SIZE} --n_val ${V_SIZE}; 
